...
function nerveZoom(zoomFactor, targetX = window.innerWidth / 2, targetY = window.innerHeight / 2, duration = 1000) {
...
function nervePan(deltaX, deltaY, duration = 1000) {
...
function nerveRotate(rotationAngle, pivotX, pivotY, duration = 1000) {
...
function nerveMovement(movementTypes = [], customZoomParams = {}, customPanParams = {}, customRotateParams = {}, customDuration = 1000) {
...
function nerveSetMandelbrotCoords(zoomMagnitude, panReal, panImaginary, speed = 0.1) {
...
function nerveZoomToNodeTitle(nodeOrTitle, zoomLevel = 1.0) {
    return new Promise((resolve) => {
        activeAnimationsCount++;
        autopilotReferenceFrame = undefined;
        let node;
...
async function nerveSearchNotes(searchTerm, maxNodesOverride = null) {
...
async function nerveSearchAndZoom(searchTerm, maxNodesOverride = null, zoomLevel = 1.0, delayBetweenNodes = 2000) {
    return new Promise(async (resolve, reject) => {
        try {
            activeAnimationsCount++;

            // Search for nodes based on the searchTerm
            const matchedNodes = await nerveSearchNotes(searchTerm, maxNodesOverride);

            // Loop through each matched node and zoom to it
            for (const node of matchedNodes) {
                await nerveZoomToNodeTitle(node, zoomLevel);
...
// Example usage
// nerveSearchAndZoom("desired search term", null, 1.0, 3000);

function nerveResetView(animate = true, duration = 2000) {
    return new Promise((resolve, reject) => {
...
function nerveGetMandelbrotCoords(forFunctionCall = false) {
...
function nerveReceiveCurrentView() {
    // Get current coordinates in standard format
    const standardCoords = nerveGetMandelbrotCoords();

    // Get current coordinates in function call format
    const functionCallFormat = nerveGetMandelbrotCoords(true);
...
function nerveSaveCurrentView() {
    const view = nerveReceiveCurrentView();
...
function nerveReturnToSavedView(savedView, animate = true, speed = 0.0001) {
    if (savedView && savedView.standardCoords) {
        ...
        // Call nerveSetMandelbrotCoords with the parsed coordinates
        nerveSetMandelbrotCoords(
...
function nerveDelay(delay) {
    return new Promise(resolve => setTimeout(resolve, delay));
}

// Verbose Schema
async function nerveAnimationQueue(animations) {
...
async function nerveQueueAnimations(animations) {
    ...
    await nerveAnimationQueue(transformedAnimations);
...
function nerveCaptureScreenshot() {
...
document.getElementById('screenshotButton').addEventListener('click', nerveCaptureScreenshot);


async function nerveReturnScreenshot() {
    return new Promise(async (resolve, reject) => {
        if (window.startedViaPlaywright) {
            // Playwright controlled session, use fetch to request screenshot
            fetch('http://localhost:8081/screenshot')
...
async function nerveCallMovementAi(movementIntention, totalIterations = 1, currentIteration = 0) {
    if (currentIteration < totalIterations) {
        const screenshotBase64 = await nerveReturnScreenshot();
        ...
        if (screenshotBase64) {
            ...
            let messages = [
                {
                    role: 'system',
                    content: nerveNeuralVisionPrompt
                },
                {
                    role: 'system',
            ...
            try {
                await callVisionModel(messages, async () => {
                    runNerveCode(true); // Run code with increment and decrement of activeAnimations.

                    // Wait for all animations to complete
                    await waitForAllAnimations();
                    console.log(`awaited`);
                    // Recursive call for the next iteration
                    await nerveCallMovementAi(movementIntention, totalIterations, currentIteration + 1);
                });
            } catch (error) {
...
async function nervePromptZettelkasten(message) {
    activeAnimationsCount++;
...
function nerveGetUserResponse(message) {
...
function nerveAddNote(nodeTitle, nodeText) {
    return new Promise((resolve) => {
        activeAnimationsCount++;
        let formattedNodeTitle = nodeTitle.replace(/\n/g, ' ');
        formattedNodeTitle = nerveGetUniqueNodeTitle(formattedNodeTitle);
...
function nerveGetUniqueNodeTitle(baseTitle) {
    let counter = 2;
...
registerFunctions([
    {
        baseFunctionName: 'nerveAddNote',
        baseFunction: nerveAddNote,
        alternateNames: ['addNote', 'createNote', 'zettelkastenAddNote', `promptNote`]
    },
    {
        baseFunctionName: 'nervePromptZettelkasten',
        baseFunction: nervePromptZettelkasten,
        alternateNames: ['promptZettelkasten', 'zettelkastenPrompt', 'promptZettelkastenAi', 'callZettelkastenAi', `zettelkastenAi`]
    },
    {
        baseFunctionName: 'nerveGetUserResponse',
        baseFunction: nerveGetUserResponse,
        alternateNames: ['getUserResponse', 'promptUser', 'requestUserResponse']
    },
    {
        baseFunctionName: 'nerveZoomToNodeTitle',
        baseFunction: nerveZoomToNodeTitle,
        alternateNames: ['zoomToNodeTitle', 'focusNode', 'zoomToNote', 'zoomToNoteByTitle', `zoomToNode`]
    },
    {
        baseFunctionName: 'nerveCallMovementAi',
        baseFunction: nerveCallMovementAi,
        alternateNames: ['callMovementAi', 'promptMovementAi', 'initiateMovementAi']
    },
    {
        baseFunctionName: 'nerveQueueAnimations',
        baseFunction: nerveQueueAnimations,
        alternateNames: ['queueAnimations', 'performSequence', 'nervePerformSequence']
    },
    {
        baseFunctionName: 'nerveResetView',
        baseFunction: nerveResetView,
        alternateNames: ['resetView', 'returnToStart', 'reinitializeView']
    },
    {
        baseFunctionName: 'nerveSetMandelbrotCoords',
        baseFunction: nerveSetMandelbrotCoords,
        alternateNames: ['setMandelbrotCoords', 'updateMandelbrotPosition', 'mandelbrotCoords']
    },
    {
        baseFunctionName: 'nerveMovement',
        baseFunction: nerveMovement,
        alternateNames: ['movement', 'startMovement', 'performMovement']
    },
    {
        baseFunctionName: 'nerveDelay',
        baseFunction: nerveDelay,
        alternateNames: ['delay', 'setDelay']
    },
    {
        baseFunctionName: 'nerveSearchNotes',
        baseFunction: nerveSearchNotes,
        alternateNames: ['searchNotes', 'returnSearchedNodes', `searchNodes`]
    },
    {
        baseFunctionName: 'nerveSearchAndZoom',
        baseFunction: nerveSearchAndZoom,
