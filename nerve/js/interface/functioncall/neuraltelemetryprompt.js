...
class NeuralTelemetry {
    ...
    getCurrentMandelbrotCoords() {
        // Assuming nerveGetMandelbrotCoords is a globally available function
        const coords = nerveGetMandelbrotCoords();
        return {
            zoom: coords.zoom,
            pan: coords.pan
        };
...
async function nerveExploreSequence() {
    try {
        const animations = [
        ...
        console.error("An error occurred during the Nerve exploration sequence:", error);
    }
}

nerveExploreSequence()`
        ],
...
function functionBasePrompt() {
    return `/* Nerve API Documentation
You write code that is executed within the fractal mind mapping interface, Nerve.
...
function neuralApiPrompt() {
    ...
    prompt += `\n/* The creation of text notes and prompts to the zettelkasten can be called BOTH inside and/or outside of Nerve's async function performSequence(animations)
...
You TAKE ACTION based off the provided SCREENSHOTS of Nerve's interface.`;
    return prompt;
}

// Usage
const nerveNeuralApiPrompt = neuralApiPrompt();
const nerveNeuralVisionPrompt = visionPrompt();

// Example console log
//console.log(nerveNeuralApiPrompt);
//console.log(nerveNeuralVisionPrompt);