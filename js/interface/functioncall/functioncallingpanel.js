        ...
        token: function (stream, state) {
            ...
            if (stream.match(/^\w+/)) {
                ...
                if (functionNameList.includes(currentToken)) {
                    return "nerve-function-name"; // Custom style for function names
                }
...
const nerveFunctionCM = CodeMirror.fromTextArea(document.getElementById('nerve-function-cm'), {
...
nerveFunctionCM.display.wrapper.classList.add('nerve-function-cm-style');
// Function to update the content of the CodeMirror instance
function updateNerveFunctionCMContent(content) {
    nerveFunctionCM.setValue(content);
    nerveFunctionCM.refresh();
}

let userScrolledUpFunctionPanel = false;

// Scroll event listener
nerveFunctionCM.on("scroll", function () {
    var scrollInfo = nerveFunctionCM.getScrollInfo();
    var atBottom = scrollInfo.height - scrollInfo.top - scrollInfo.clientHeight < 1;
    userScrolledUpFunctionPanel = !atBottom;
});

// Change event listener
nerveFunctionCM.on("change", function () {
    if (!userScrolledUpFunctionPanel) {
        // Scroll to the bottom
        var scrollInfo = nerveFunctionCM.getScrollInfo();
        nerveFunctionCM.scrollTo(null, scrollInfo.height);
    }
});
...
function runNerveCode() {
    const code = nerveFunctionCM.getValue();
...
function addFunctionCallItem(functionName, code, isError = false) {
    const currentCoords = nerveGetMandelbrotCoords(); // Get current coordinates
...
functionRunButton.addEventListener('click', runNerveCode);
...
functionCallList.addEventListener('click', function (event) {
    ...
    if (event.target.tagName === 'DIV') {
        if (event.target.classList.contains('active-state')) {
            ...
            if (originalText) {
                updateNerveFunctionCMContent(originalText);
            }
        } else {
