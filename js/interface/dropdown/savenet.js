...
// Attach the nerveSaveEvent to the save button
document.getElementById("new-save-button").addEventListener("click", () => nerveSaveEvent());
...
function updateSavedNetworks() {
    ...
    for (let [index, save] of saves.entries()) {
        ...
        saveButton.addEventListener('click', function () {
            ...
            nerveSaveEvent(existingTitle = save.title)
        });
...
function collectAdditionalSaveObjects() {
    ...
    const mandelbrotParams = nerveGetMandelbrotCoords();
...
function restoreAdditionalSaveObjects(d) {
    ...
    if (mandelbrotSaveElement) {
        ...
        nerveSetMandelbrotCoords(mandelbrotParams.zoom, mandelbrotParams.pan.split('+i')[0], mandelbrotParams.pan.split('+i')[1]); // Direct function call using parsed params
...
function nerveSaveEvent(existingTitle = null) {
...
function autosave() {
    if (selectedSaveTitle !== null && autosaveEnabledCheckbox.checked) {
        nerveSaveEvent(selectedSaveTitle)
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (stateFromURL) {
        ...
        fetch(`/wiki/pages/nerve-wikis/${stateFromURL}.txt`)
            .then(response => {
                if (!response.ok) {
